...
class TrainLoop:
    ...
    def _sample_and_visualize(self):
        ...
        with th.no_grad():
            if self.ssc_refine:
                large_T = th.tensor([self.training_step-1] * _shape[0], device=self.device)
                batch = batch.to(self.device)
                m_t = self.diffusion.q_sample(batch, large_T)
                noise = self.ddp_model(m_t, large_T, cond['H'], cond['W'], cond['D'], cond['y']).to(self.device)
            else : noise = None
    ...
    def forward_backward(self, batch, cond):
        ...
        for i in range(0, batch.shape[0], self.microbatch):
            ...
            micro = batch.to(self.device)
            micro_cond = {}

            for k, v in cond.items():
                if (k != 'path'):
                    micro_cond[k] = v.to(self.device)
                else :
